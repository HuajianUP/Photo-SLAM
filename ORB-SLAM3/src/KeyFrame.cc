    ...
    std::vector<float> &pointsLocal)
{
    ...
    // Save the id of each MapPoint in this KF, there can be null pointer in the vector
    pixelsUndist.clear();
    pixelsUndist.reserve(N * 2);
    pointsLocal.clear();
    pointsLocal.reserve(N * 3);
    for(int i = 0; i < N; ++i)
    {
        pixelsUndist.push_back(mvKeysUn[i].pt.x);
        pixelsUndist.push_back(mvKeysUn[i].pt.y);
        if(mvpMapPoints[i])
        {
            auto pos_local = this->GetPose() * mvpMapPoints[i]->GetWorldPos();
            pointsLocal.push_back(pos_local.x());
            pointsLocal.push_back(pos_local.y());
            pointsLocal.push_back(pos_local.z());
        }
        else // If the element is null his value is -1 because all the id are positives
        {
